...
import com.xb.wechatmp.domain.weixin.menu.WxMenuButton;
import com.xb.wechatmp.domain.weixin.menu.WxMenuRule;
...
public class WxMenuGsonAdapter implements JsonSerializer<WxMenu>, JsonDeserializer<WxMenu> {
    @Override
    public WxMenu deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
       /*
        * 操蛋的微信
        * 创建菜单时是 { button : ... }
        * 查询菜单时是 { menu : { button : ... } }
        ...
        */
        JsonArray buttonsJson = jsonElement.getAsJsonObject().get("menu").getAsJsonObject().get("button").getAsJsonArray();
        return this.buildMenuFromJson(buttonsJson);
    }

    private WxMenu buildMenuFromJson(JsonArray buttonsJson) {
    ...
    private WxMenuButton convertFromJson(JsonObject json) {
    ...
    @Override
    public JsonElement serialize(WxMenu menu, Type type, JsonSerializationContext jsonSerializationContext) {
    ...
    private JsonObject convertToJson(WxMenuButton button) {
    ...
    private JsonObject convertToJson(WxMenuRule menuRule) {
